...
def 解析PDF(file_name, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt):
    ...
    i_say_show_user = f'首先你在英文语境下通读整篇文章。';
    ...
    for i in range(n_fragment):
        ...
        gpt_say = yield from request_gpt_model_in_new_thread_with_ui_alive(i_say, i_say_show_user,
                                                                           ...
                                                                           sys_prompt="Extract the main idea of this section."
                                                                           # 提示
                                                                           )
    ...
    final_results.append(f'接下来，你是一名专业的分析员，利用以上信息，使用中文回答我的问题。')

    # 接下来两句话只显示在界面上，不起实际作用
    i_say_show_user = f'接下来，你是一名专业的分析员，利用以上信息，使用中文回答我的问题。';
